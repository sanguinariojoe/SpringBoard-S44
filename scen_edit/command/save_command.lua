...
local function ModelSave(path)
    local success, mission = pcall(Model.Serialize, SB.model)
    if not success then
        Log.Error(mission)
    end
    -- Drop flags and buoys
    for key, unit in pairs(mission.unitS11N) do
        if unit.defName == "flag" or unit.defName == "buoy" then
            mission.unitS11N[key] = nil
        end
    end
    table.save(mission, path)
end
